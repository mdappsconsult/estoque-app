...
- **Raspberry Pi (rede local, usuário `kim`):** serviço **systemd** `pi-print-ws` em `~/pi-print-ws` — WebSocket **TCP 8765**, HTML → PDF (Chromium / **puppeteer-core**, `preferCSSPageSize`) → **`lp`** (CUPS). `.env` no Pi: `PRINT_WS_TOKEN`, **`CUPS_QUEUE=ZebraZD220`**. **Zebra ZD220** USB. **Túnel Cloudflare (quick):** **`cloudflared-pi-print-ws`** + **`cloudflared-quick-tunnel-sync.sh`**: chama RPC **`sync_pi_tunnel_ws_url`** com **`PI_TUNNEL_SYNC_SECRET`** da linha **`papel`** correspondente no banco; segundo Pi usa **`PI_TUNNEL_PAPEL=industria`** no `.env`. **App:** prioridade `NEXT_PUBLIC_PI_PRINT_WS_URL` (dev); senão **`config_impressao_pi`** por **`papel`** (`estoque` = Separar por Loja; `industria` = segundo Raspberry / uso futuro em Produção). **Configurações → Impressoras (Pi):** editar URL/token/fila e **Verificar agora** (`/api/impressoras/status`). Scripts: `scripts/pi-print-ws/`. Docs: `docs/IMPRESSAO_PI_ACESSO_REMOTO.md`, **`docs/RASPBERRY_INDUSTRIA_NOVO_PI.md`** (checklist novo Pi indústria). Migração **`20260406120000_config_impressao_pi_papel.sql`**.
- **“Offline / fetch failed” em Verificar agora:** não é teste da USB da Zebra — é o **Railway** tentando `GET https://…/health` no host do **túnel**. Causas típicas: URL **`wss://`** desatualizada (túnel quick novo hostname), `cloudflared` ou **`pi-print-ws`** parados no Pi, ou rede. A API passa a anexar código da causa (ex. `ENOTFOUND`). Troubleshooting: `docs/IMPRESSAO_PI_ACESSO_REMOTO.md`.
