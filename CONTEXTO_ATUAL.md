...
- **Hospedagem:** app Next.js em **Railway** via **`Dockerfile`** + **`railway.toml`** (Next **`standalone`**, `startCommand` = `node server.js`); cache de layers no build. Variáveis `NEXT_PUBLIC_SUPABASE_*` no build e runtime (`README.md`, `docs/FLUXO_ENTREGA.md`).
