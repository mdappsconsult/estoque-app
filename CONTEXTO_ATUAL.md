...
- **Raspberry Pi (rede local, usuário `kim`):** serviço **systemd** `pi-print-ws` em `~/pi-print-ws` — WebSocket **TCP 8765**, HTML → PDF (Chromium / **puppeteer-core**, `preferCSSPageSize`) → **`lp`** (CUPS). `.env` no Pi: `PRINT_WS_TOKEN`, **`CUPS_QUEUE=ZebraZD220`**. **Zebra ZD220** USB. **Túnel Cloudflare (quick):** **`cloudflared-pi-print-ws`** pode usar **`cloudflared-quick-tunnel-sync.sh`**: lê a URL `https://*.trycloudflare.com` nos logs e chama RPC **`sync_pi_tunnel_ws_url`** (anon + **`PI_TUNNEL_SYNC_SECRET`** = **`tunnel_sync_secret`** no banco, só via SQL Editor) para gravar **`wss://`** em `config_impressao_pi`. **Túnel nomeado** (Zero Trust) evita troca de hostname. **App:** bridge — **prioridade** `NEXT_PUBLIC_PI_PRINT_WS_URL` (dev); senão **`config_impressao_pi`**. **Separar por Loja** + **Teste de impressão:** **Pi / Zebra**. Scripts: `scripts/pi-print-ws/`. Doc: `docs/IMPRESSAO_PI_ACESSO_REMOTO.md`. Código: `pi-print-ws-client.ts`, `usePiPrintBridgeConfig`, `gerarDocumentoHtmlEtiquetas`.
- Rota **`/teste-impressao-etiqueta`**: gera amostra fictícia (`gerarEtiquetasDemonstracaoImpressao`) e abre a mesma janela de impressão — calibragem física sem tocar no banco; botão **Imprimir na estação (Pi / Zebra)** com env ou **`config_impressao_pi`**; link na tela **Etiquetas**; permissões iguais a **Etiquetas**.
