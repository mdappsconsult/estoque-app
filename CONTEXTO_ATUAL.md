...
- **Hospedagem:** app Next.js em **Railway** com build **Railpack** (sem Docker no repo). Variáveis `NEXT_PUBLIC_SUPABASE_*` no build e runtime. Um único gatilho por mudança: push **ou** `npm run railway:release` — não os dois em sequência. Vários **`QUEUED`**: **`RAILWAY_PROJECT_TOKEN`** (token do projeto) ou **`RAILWAY_TOKEN`** (conta) + **`npm run railway:prune-queued`** (ver `docs/FLUXO_ENTREGA.md`); não commitar token. Diagnóstico: **`npm run railway:diagnose`**; deploys Docker presos: cancelar no dashboard.
