...
- **Hospedagem:** app Next.js em **Railway** com build **Railpack** (sem Docker no repo). Variáveis `NEXT_PUBLIC_SUPABASE_*` no build e runtime. Um único gatilho por mudança: push **ou** `npm run railway:release` — não os dois em sequência (`README.md`, `docs/FLUXO_ENTREGA.md`).
