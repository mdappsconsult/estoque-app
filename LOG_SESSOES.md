...
- **Service `services/protocolos.ts`**: `Protocolo.foto_paths: string[]` + `AbrirProtocoloInput.foto_paths?: string[]` (campo `foto_path` antigo mantido como compat, normalizado por helper `resolverFotoPaths` que aceita ambas formas e valida `<=3`). `abrirProtocolo` e `abrirProtocoloComClient` agora gravam `foto_path = foto_paths[0] ?? null` e `foto_paths = [..]` (dual-write para rollback seguro). Auditoria `ABRIR_PROTOCOLO` troca `tem_foto` por `qtd_fotos`. `SELECT_LISTA` ganhou `foto_paths`.
- **Rota `POST /api/admin/excluir-protocolo`**: remoção do bucket agora usa **união de `foto_paths` + `foto_path` legado** (Set para deduplicar). Auditoria `EXCLUIR_PROTOCOLO` grava `qtd_fotos` e `foto_paths` (lista completa apagada).
- **UI `ModalAbrirProtocolo`** ([`src/app/protocolos/page.tsx`](src/app/protocolos/page.tsx)): trocou `arquivo: File | null` por `fotos: FotoPedido[]` (max 3). Grid 3×1 de thumbs com botão **X** vermelho redondo em cada e badge `1/3 | 2/3 | 3/3`. Botão único «Tirar foto» (ou «Tirar mais uma (N/3)») abre a câmera; link «Escolher da galeria» continua. Atingiu o limite → o botão some e aparece nota «remova uma para trocar». Cleanup de `URL.createObjectURL` por slot em `removerFoto` e batch no unmount. Upload **sequencial** no `handleEnviar` (mais simples para tratar erro e dar feedback claro); cada foto é comprimida com `comprimirFotoProtocolo` antes de ir para `/api/operacional/upload-protocolo-foto`. Ícone `RefreshCw` saiu do import (era do botão «Outra foto» removido).
- **UI `ModalDetalheProtocolo`**: `fotoUrl: string | null` virou `fotosUrls: string[]` + `lightboxIndex: number | null`. Caminhos efetivos resolvidos por memo `fotosPaths` (usa `foto_paths`, cai em `foto_path` para registros antigos). URLs carregadas em **paralelo** com `Promise.all` (uma chamada à API de URL assinada por foto). Grid 1× (cheio) quando há 1 foto, **2/3 colunas** com aspect-square quando há mais. Cada thumb é um `<button>` que abre o **lightbox em overlay fullscreen** (`fixed inset-0 z-[100] bg-black/90`) com a foto em `max-w-[95vw] max-h-[90vh] object-contain`. Clique fora ou no X fecha; ‹ › navegam circularmente; teclado `Esc`, `ArrowLeft`, `ArrowRight` espelham os botões; contador `1 / 3` na parte de baixo.
- **Chip de foto no card da lista**: antes mostrava só «foto»; agora mostra «foto» quando há 1, «N fotos» quando há mais (lê `p.foto_paths.length`, cai em `foto_path` se vier vazio).
- **Compatibilidade & rollback:** dual-write garante que se a coluna `foto_paths` for dropada num rollback de emergência, o `foto_path` antigo ainda terá a primeira foto. Leituras antigas (`SELECT … foto_path`) continuam funcionando. O `comprimirFotoProtocolo` (canvas client-side, max 1600px, JPEG q=0.7) não mudou; cada foto fica ~150-300 KB.
- **Validação:** `npm run lint` OK, `npm run build` OK. Teste manual sugerido: (1) abrir pedido novo com 3 fotos diferentes, (2) abrir o detalhe e tocar cada thumb pra ver em tamanho real, (3) testar teclado ‹/›, (4) admin excluir o pedido e conferir que `auditoria.acao='EXCLUIR_PROTOCOLO'` lista as 3 paths e que o bucket fica limpo.

### Sessão - 2026-05-21 - Excluir pedido de protocolagem (admin)
- **Pedido:** «cria para o administrador opção de excluir os pedidos de protocolagem».
- **Modelo do banco** (`supabase/migrations/20260521120000_protocolos.sql`): `protocolo_comentarios.protocolo_id` é FK com `ON DELETE CASCADE` (comentários somem junto). `protocolos.reaberto_de` é FK com `ON DELETE SET NULL` (protocolos reabertos a partir de um excluído não perdem referência forte, só o ponteiro vira NULL). `auditoria.detalhes->>'protocolo_id'` **não é FK** — registros históricos ficam preservados depois da exclusão (importante para rastrear quando um pedido foi aberto/aceito/encerrado mesmo após deleção).
- **Rota nova** [`src/app/api/admin/excluir-protocolo/route.ts`](src/app/api/admin/excluir-protocolo/route.ts): `POST { protocoloId, actorId }`. Mesma anatomia das outras rotas `/api/admin/*`: cria `createSupabaseAdmin()` (Service Role), valida `actor.perfil === 'ADMIN_MASTER' && status='ativo'` (403 caso contrário), busca o protocolo (404 se sumiu), faz `DELETE FROM protocolos WHERE id=?`, remove a foto via `admin.storage.from('protocolos-fotos').remove([foto_path])` se houver (best-effort — log mas não bloqueia) e grava auditoria **`EXCLUIR_PROTOCOLO`** via `registrarAuditoria(..., admin)` com snapshot `{ protocolo_id, numero, titulo, status, prioridade, aberto_por, tinha_foto, foto_path, created_at }`.
- **Service** [`src/lib/services/protocolos.ts`](src/lib/services/protocolos.ts) ganhou `excluirProtocoloAdmin(protocoloId, actorId)` — wrapper `fetch('/api/admin/excluir-protocolo', POST)` que lança o `error` da rota para a UI mostrar via `errMessage`.
- **UI** [`src/app/protocolos/page.tsx`](src/app/protocolos/page.tsx): no `ModalDetalheProtocolo`, abaixo de `{acoesGestao}` e `{acaoOperador}`, botão **«Excluir pedido (admin)»** com `Trash2` quando `usuario.perfil === 'ADMIN_MASTER'`. Confirmação dupla (`window.confirm` mostra `#numero` + título e avisa que comentários e foto somem). Após sucesso: `onAcaoConcluida()` (refetcha a lista) + `onClose()`. Handler `onExcluirProtocolo` reaproveita `acaoEmAndamento` para evitar duplo clique.
- **Não-objetivo:** soft-delete não foi implementado — o pedido é o uso administrativo (limpar lixo / pedidos abertos por engano). Quem precisa de rastreio histórico já tem `auditoria` (snapshot completo).
- **Validação:** `npm run lint` OK, `npm run build` OK (rota nova aparece em `/api/admin/excluir-protocolo` como Dynamic). Sem migração de banco. Teste manual: abrir um protocolo de teste com Marco e excluir; conferir que `auditoria.acao='EXCLUIR_PROTOCOLO'` ganha 1 linha e que `protocolos`/`protocolo_comentarios` ficaram limpas.
...
- **Helper compartilhada `perfilEscopoLojaDestino`** em [`src/lib/operador-loja-scope.ts`](src/lib/operador-loja-scope.ts): retorna `true` para `OPERATOR_STORE` e `RECEIVING_ASSIST`. Refatorei `filtrarRecebimentoPorLoja`, `idLocalLojaOperadora`, `filtrarItensPorLojaOperadora` e `filtrarRemessasMatrizAguardandoMotorista` para usar a helper (RECEIVING_ASSIST passa a respeitar o mesmo filtro `destino_id = local_padrao_id` do operador de loja). Em [`src/app/recebimento/page.tsx`](src/app/recebimento/page.tsx) troquei todos os `usuario.perfil === 'OPERATOR_STORE'` que eram **escopo de loja** (filtros, mensagens «Lista remessas da indústria para a sua loja…», bloco de remessas já encerradas, modo multi-remessas que pré-marca todas, fallback de loja sem `local_padrao_id`) pela variável `escopoLoja = perfilEscopoLojaDestino(usuario?.perfil)`.
- **Cadastros → Usuários** ([`src/app/cadastros/usuarios/page.tsx`](src/app/cadastros/usuarios/page.tsx)): adicionado o item «Ajudante de recebimento» no `<Select>` de perfil; introduzida helper `ehPerfilDeLoja(perfil)` (`OPERATOR_STORE` **ou** `RECEIVING_ASSIST`) que controla obrigatoriedade do campo «Loja de atuação», geração de `localOptions` filtrando `tipo === 'STORE'` e o `disabled` do botão Criar/Salvar. `idsLocaisPermitidosParaPerfil` também passa por essa helper. Badge do perfil pintado como `success` (mesmo verde do operador de loja). [`src/app/configuracoes/perfil/page.tsx`](src/app/configuracoes/perfil/page.tsx) ganhou a entrada no `perfilLabel` para mostrar «Ajudante de recebimento» na tela do usuário.
- **Banco** — duas operações via MCP no ref do `.env.local`:
  - Migração **`20260521170000_perfil_receiving_assist.sql`**: dropou o `CHECK` antigo `usuarios_perfil_check` (que listava 6 perfis) e recriou incluindo `'RECEIVING_ASSIST'`. Aplicada com `apply_migration`.
  - `INSERT` em `public.usuarios` + `public.credenciais_login_operacional` numa CTE só (RETURNING dos novos `id`, hash bcrypt gerado por `extensions.crypt(senha, extensions.gen_salt('bf', 10))` — formato `$2a$` que o `bcryptjs` aceita no `validarCredencialOperacional`). Resultado: 3 usuários ativos com `perfil = 'RECEIVING_ASSIST'`, `local_padrao_id = 97cff1c2-6add-4f3a-a7ba-6024d6fd1fb6` (Delivery), `login_operacional` `ajudante1|2|3` e telefones placeholder `5500000001{01,02,03}`. Conferi com `SELECT … FROM usuarios LEFT JOIN credenciais_login_operacional` — todos com `tem_senha=true`.
- **Tipos TS** — `src/types/database.ts` ganhou `'RECEIVING_ASSIST'` no union de `usuarios.perfil` (Row/Insert/Update).
- **O que ele NÃO pode fazer:** confirmar entrega sem escanear, encerrar com divergência, ver estoque/validades, separar por loja, etiquetas, protocolos, cadastros, dashboard, viagem, qrcode genérico. O serviço `receberTransferencia` continua exigindo `ADMIN_MASTER` para `encerrarComDivergencia` — RECEIVING_ASSIST cai na mesma mensagem que um `OPERATOR_STORE` veria.
- **Validação:** `npm run lint` OK (warning de `useMemo` corrigido com `escopoLoja` no array de deps), `npm run build` OK (47 rotas estáticas). Teste de login com os 3 ajudantes fica para a primeira oportunidade no celular real.

### Sessão - 2026-05-21 - Recebimento colaborativo (vários bipadores em paralelo)
- **Pedido:** "no receber entrega, hoje só um funcionário bipa e leva 2 h — eu queria que vários ajudassem em tempo real". Decisões alinhadas no plan: (a) acesso livre — qualquer operador da loja destino entra na mesma remessa, sem etapa de "liberar ajuda"; (b) sem desbipar — o sistema já rejeita QR fora da `transferencia_itens` esperada, então um bip que entra é sempre legítimo; (c) escopo só do fluxo SEP (matriz→loja e loja→loja), `RecebimentoDiretoCard` e `EnvioDiretoConferenciaCard` ficam como estão.
- **Mudança de paradigma:** antes, os bips ficavam só no React state (`itensRecebidos`) e o "Confirmar recebimento" rodava tudo em batch via `receberTransferencia`. Agora cada bip vai direto ao banco; o "Confirmar" só vira o status.
- **Migração `20260521150000_transferencia_itens_recebido_por.sql`** (aplicada via MCP no ref do `.env.local`): `transferencia_itens` ganhou `recebido_por_usuario_id uuid REFERENCES usuarios(id)` + `recebido_em timestamptz` (ambos nullable; linhas antigas ficam NULL preservando o legado batch). Índice parcial `transferencia_itens_remessa_pendente_idx ON (transferencia_id) WHERE recebido = false` para acelerar o "X de Y" do painel.
- **Serviço novo `bipQrRecebimentoColaborativo`** em [`src/lib/services/transferencias.ts`](src/lib/services/transferencias.ts): recebe `{ transferenciaId, codigoQr, localDestinoId, usuarioId }`. Valida remessa (status `AWAITING_ACCEPT|ACCEPTED|IN_TRANSIT`, `destino_id` bate, **rejeita `modo_bip_loja`** — esse fluxo continua no `EnvioDiretoConferenciaCard`). Resolve QR (token completo ou curto). Verifica vínculo `transferencia_itens` da remessa: se não existe, mensagem operacional pedindo «Envio direto da produção» ou separação correta; se existe e já está `recebido=true`, busca o `usuarios.nome` do auditor e mostra «Este QR já foi bipado por **Joana**». **UPDATE condicional** `SET recebido=true, recebido_por_usuario_id, recebido_em=now() WHERE transferencia_id, item_id AND recebido=false RETURNING id` — se 0 rows (corrida entre 2 aparelhos no mesmo QR), lança «Este QR já foi bipado por outro funcionário». Move item (`local_atual_id`, `EM_ESTOQUE`), chama `recalcularEstoqueProduto` (versão completa com massa, mesmo padrão de `bipQrEnvioDireto`), conta `recebidos`/`total` no servidor para o caller saber se já completou e grava auditoria **`BIP_RECEBIMENTO_COLABORATIVO`** com `{ transferencia_id, recebidos, total, token_short }`.
- **Serviço novo `fecharRemessaRecebimentoColaborativo`**: chamado pelo botão «Confirmar recebimento» quando todos já bipados. Confere `count(transferencia_itens WHERE recebido=false) = 0`, vira status para `DELIVERED`, grava `RECEBER_TRANSFERENCIA` com `detalhes.modo: 'colaborativo'`. Não move itens nem mexe em estoque (já aconteceu por bip).
- **`receberTransferencia` legado-batch preservado** para botões admin («Confirmar entrega inteira sem escanear», «Encerrar com divergência»): o UPDATE em `transferencia_itens` agora inclui `.eq('recebido', false)` — preserva `recebido_por_usuario_id` das linhas já bipadas colaborativamente (não sobrescreve o auditor original quando o admin fecha um restante).
- **UI `/recebimento`** ([`src/app/recebimento/page.tsx`](src/app/recebimento/page.tsx)): removido o state local `itensRecebidos`. `itensEsperados` virou unificado (com `recebido`, `recebido_em`, `recebedor_nome`) carregado por fetch + assinatura `postgres_changes` em `transferencia_itens` (canal sem filtro do Realtime — filtra client-side pelas remessas selecionadas, debounce 200 ms para absorver rajadas de bips). Lista mostra «bipado por **Joana** às 14:32» (`usuarios!recebido_por_usuario_id(nome)` embedded). Aviso antigo «Os QRs escaneados ficam só neste aparelho» virou um banner azul com ícone `Users` dizendo «Vários funcionários da loja podem bipar ao mesmo tempo — cada QR lido aparece em todos os aparelhos na hora». Lista «Itens recebidos» abaixo do scanner removida (redundante com o painel «Itens esperados» que já mostra status). `escanear()` agora chama `bipQrRecebimentoColaborativo`; no modo agrupado, tenta cada remessa selecionada até alguma aceitar (servidor faz a validação).
- **Permissões inalteradas:** `/recebimento` segue em `OPERATOR_STORE` / `MANAGER` / `ADMIN_MASTER`. Operador de loja só vê remessas com `destino_id = local_padrao_id` (filtro PostgREST mantido). Não houve mudança em RLS — o controle continua app-side, e o canal realtime em `transferencia_itens` é o mesmo padrão usado em outras telas.
- **Cenário de corrida testável:** Joana e Lara escaneiam o mesmo QR no mesmo segundo → o `UPDATE WHERE recebido=false` afeta 1 linha; o segundo bip recebe 0 rows e lança «Este QR já foi bipado por outro funcionário». QRs diferentes em paralelo entram normalmente.
- **Validação:** `npm run lint` OK, `npm run build` OK (warning de cast resolvido com `unknown as TiRow[]` por causa de embeds aninhados de PostgREST). Teste manual com 2 navegadores fica para a próxima vez que houver remessa SEP em trânsito real (recomendado: abrir `/recebimento` em dois contextos, bipar em paralelo, observar o «X de Y» atualizando, confirmar fechamento e ver realtime no segundo aparelho).
- **Trade-off conhecido:** se o WebSocket do Realtime cair em um celular, esse celular pode mostrar uma lista defasada por alguns segundos até o canal religar. Se o usuário tocar em «Confirmar» antes do refetch, o servidor recusa com «Ainda há N QR(s) pendente(s)» — efeito colateral desejado (nada vaza).
...
- **UI loja (`/recebimento`):** novo card **`RecebimentoDiretoCard`** sempre no topo para `OPERATOR_STORE`/`MANAGER`/`ADMIN_MASTER` com `local_padrao_id`. Inicia colapsado («Chegou balde direto da indústria? Bipar»), abre leitor de QR + digitação manual, lista bipados na sessão com horário. Não precisa selecionar remessa.
- **UI indústria (`/envio-direto-producao`):** banner azul «Caminho simples (recomendado)» no topo orientando o uso do bip direto. Nova seção **«Saídas recentes (bip direto na loja)»** agregando últimas 48 h por loja+produto (`listarSaidasAvulsasRecentes` filtra `modo_bip_loja=true AND status=DELIVERED AND qty=1 AND origem=indústria`). Formulário **«Criar envio com plano»** virou `<details>` recolhido — uso opcional para planejar quantidade antes.
- **Risco/decisão:** quem tem o QR físico pode receber (modelo «posse vale»). Bloqueia recebimento se o item já estiver em outra remessa aberta (`AWAITING_ACCEPT`/`ACCEPTED`/`IN_TRANSIT`) ou se não estiver em uma `WAREHOUSE` — evita pegar balde já reservado para outra loja ou vindo de outra loja.
- **Validação:** `npm run lint` OK; `npm run build` OK.
...
- **Validação:** `npm run lint`, `npm run build`.
- **Deploy:** commit `821ab7f` em `main` — `git push origin main` (Railway pelo repo; evitar `railway up` em duplicata — ver `docs/FLUXO_ENTREGA.md`).
