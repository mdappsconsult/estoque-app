...
- **Variáveis:** `NEXT_PUBLIC_SUPABASE_URL` e `NEXT_PUBLIC_SUPABASE_ANON_KEY` no serviço (**Build** e **Runtime**). **`SUPABASE_SERVICE_ROLE_KEY` também é obrigatória no runtime** (login operacional e API de credenciais); sem ela o app responde erro ao entrar. Copie do Supabase (Settings → API → *service role*, nunca commitar).
