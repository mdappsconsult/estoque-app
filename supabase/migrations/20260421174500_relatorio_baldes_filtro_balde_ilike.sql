-- Relatório baldes: filtro por nome usando ILIKE (regex \\m/\\M não bateu em nomes reais como "Açaí Balde 11L").
...
COMMENT ON FUNCTION public.relatorio_baldes(date, date, uuid, uuid, boolean, uuid) IS
  'Relatório baldes: por loja/produto, saldo agora (indústria/loja/trânsito) e baixas no período. Filtro "balde" via ILIKE.';
