-- Relatório baldes: não exigir itens.producao_id (baldes podem vir de compra/separação na indústria).
...
COMMENT ON FUNCTION public.relatorio_baldes(date, date, uuid, uuid, boolean, uuid) IS
  'Relatório baldes: por loja/produto, saldo agora (indústria/loja/trânsito) e baixas no período. Não exige producao_id.';
